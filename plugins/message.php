...
if (!isset($_SESSION['staff_email'])) {
	...
	$add_cred = "SELECT * from staff_account where staff_email='$staff_email'";
		...
		<div id="page-content-wrapper">
												...
												<ul class="nav navbar-nav">
													...
													<li data-toggle="modal" data-toggle="modal" data-target="#new_msg"><a href="#">
														...
														<?php while($row = $result->fetch_assoc()){
															?>
															<span style="background:#42ba96; padding:8px; border-radius: 5px;" class="badge" ><?php echo $row['c']?>&nbsp;<i class="fa fa-envelope"></i></span>&nbsp;New Messages
														<?php } ?>
													</a></li>
																	...
																	<div class="row">
																		<div class="col-md-12">
																			...
																			<div class="msg-mini-chat">
																				...
																				$msg = $db->prepare("SELECT * from patient_concern WHERE msg_status=? ORDER BY concern_id DESC");
													...
													<li data-toggle="modal"  data-toggle="modal" data-target="#unread_msg"><a href="#">
														...
														<?php while($row = $result->fetch_assoc()){
															?>
															<span style="background:#0275d8; padding:8px; border-radius:5px" class="badge" ><?php echo $row['c']?>&nbsp;<i class="fa fa-envelope"></i></span>&nbsp;Unread Messages
														<?php } ?>
													</a></li>
													...
													<li data-toggle="modal"  data-toggle="modal" data-target="#arch_msg"><a href="#">
														...
														<?php while($row = $result->fetch_assoc()){
															?>
															<span style="background:#ff9966; padding:8px; border-radius:5px" class="badge" ><?php echo $row['c']?>&nbsp;<i class="fa fa-archive"></i></span>&nbsp;Archives
														<?php } ?>
													</a></li>
													<!-- Modal archived messages-->
										...
										<div class="row">
											<div class="col-md-4">
												<form method="GET">
													...
													&emsp;<button type="submit" title="Delete" name="delete_all" id="fa-trash" onclick="return confirm('Are you sure you want to delete selected messages?')" class="btn btn-sm btn-danger" ><i class="fa fa-trash"></i></button>

													&emsp;<button type="submit" title="Archive" name="archive_all" id="fa-arch" onclick="return confirm('Are you sure you want to archive selected messages?')" class="btn btn-sm btn-warning"><i class="fa fa-archive"></i></button>

													&emsp;<button type="submit" title="Mark as unread" name="unread_all" id="fa-envelope" onclick="return confirm('Are you sure you want to mark as unread the selected messages?')" class="btn btn-sm btn-primary"><i class="fa fa-envelope"></i></button>
													...
													<div class="msg-mini-chat">
														...
														$msg = $db->prepare("SELECT * from patient_concern WHERE msg_status!=? AND msg_status!=? ORDER BY concern_id DESC");
																				...
																				<div class="msg-min-info">
																					<div class="row">
																						<div class="col-md-12">
																							...
																							<h5 class="con_min_name">
																								<input type="checkbox" class="select-msg" onclick="selectChkbxs()" name="msg_id[]" id="chkbx" value="<?php echo $msg_id;?>" style="width: 15px;height: 15px;">
																								<?php 
							...
							<b><span class="msg_created"><?php echo $rows['subject'] ?></span></b>
														...
														if (isset($_GET['delete_all'])) {
															if (!empty($_GET['msg_id'])) {
																...
																$sql = "UPDATE patient_concern SET msg_status='ARCHIVED' WHERE concern_id IN($extract_id)";
														...
														if (isset($_GET['archive_all'])) {
															if (!empty($_GET['msg_id'])) {
																$all_id = $_GET['msg_id'];
																$extract_id = implode(',', $all_id);
																$sql = "UPDATE patient_concern SET msg_status='ARCHIVED_MSG' WHERE concern_id IN($extract_id)";
																$result = mysqli_query($db, $sql);
																if($result){
														...
														if (isset($_GET['unread_all'])) {
															if (!empty($_GET['msg_id'])) {
																$all_id = $_GET['msg_id'];
																$extract_id = implode(',', $all_id);
																$sql = "UPDATE patient_concern SET msg_status='UNREAD_MSG' WHERE concern_id IN($extract_id)";
																$result = mysqli_query($db, $sql);
																if($result){
											...
											if (isset($_GET['msg_content'])) {
												...
												$read = "READ";
												$read_sql = $db->prepare("UPDATE patient_concern SET msg_status=? WHERE concern_id=?");
												$read_sql->bind_param('ss', $read, $msg_content);
												$read_sql->execute();
								
											}

											$msg = $db->prepare("SELECT * from patient_concern WHERE concern_id=? ");
											...
											if (isset($_POST['send_reply'])) {
												...
												$sql = $db->prepare("SELECT * FROM patient_concern WHERE concern_number = ? AND concern_id =?");
												...
												if ($result->num_rows > 0) {
													while($row = $result->fetch_assoc()){
														...
														if ($output) {
															...
															$syslog->bind_param('ssssss', $staff_name, $staff_email, $date_created, $msg_con_id, $field, $description );
															$syslog->execute();
															echo "<script>
															$(document).ready(function() {
																...
																$msg_status = 'REPLIED';
																$stmt = $db->prepare("UPDATE patient_concern SET msg_status=?, reply=? WHERE concern_id =? AND concern_number =?");
            ...
            // echo 'Message has been sent';
													...
													if (isset($_POST['archive_msg'])) {
														...
														$prev_log_sql = $db->prepare("SELECT * from patient_concern where concern_id=?");
														...
														if ($prev_log_sql_result->num_rows  > 0) {
															while($row = $prev_log_sql_result->fetch_assoc()){
																...
																$prev_log->bind_param('ssss', $msg_id, $staff_name, $date_created, $prev_log_desc );
																$prev_log->execute();
															}
														}

														$arch_msg = $db->prepare('UPDATE patient_concern SET msg_status =? WHERE concern_id=? AND concern_number=?');
														...
														if ($arch_msg) {
															...
															$syslog = $db->prepare('INSERT INTO systemlogs (name, email, date_created, uniqueID, field, description)VALUES(?,?,?,?,?,?)');
															$syslog->bind_param('ssssss', $staff_name, $staff_email, $date_created, $msg_id, $field, $description );
															$syslog->execute();
															echo "<script>
															$(document).ready(function() {
																msg_archive();
