...
if(isset($_POST['mediaupload'])){
    ...
    if($title == "" || $component == "" || $duration == "" || $budget == "" || $fund == "" || $research == ""){
        echo "<script>alert('Fill all fields correctly');window.location.href = 'accomplished.php';</script>";
    }else{
        $random = random_int(100000, 999999);
        $UniqueID = 'BR'.$random;

        $Proposal_new_name = 'Copy_of_Proposal'.'.'.$title.'.'.$Proposal_file_ex_loc;
        $Proposal_video_path_sa_buhay_niya = 'files/accomplished/'.$Proposal_new_name;
        ...
        $HGDG_video_path_sa_buhay_niya = 'files/accomplished/'.$HGDG_new_name;
        move_uploaded_file($HGDG_file_tmp, $HGDG_video_path_sa_buhay_niya);

        $paper_new_name = 'Copy of Paper'.'.'.$title.'.'.$paper_file_ex_loc;
        $paper_video_path_sa_buhay_niya = 'files/accomplished/'.$paper_new_name;
        ...
        $PIMME_video_path_sa_buhay_niya = 'files/accomplished/'.$PIMME_new_name;
        move_uploaded_file($PIMME_file_tmp, $PIMME_video_path_sa_buhay_niya);

        $insert_research = "INSERT INTO tbl_accomplished (fldUID, fldTitle, fldDescription, fldResearch, fldDFrom, fldDTo, fldDuration, fldBudget, fldFund, fldProposal, fldHgdg, fldPaper, fldPIMME, fldStatus, fldFrom) VALUES ('$UniqueID', '$title', '$component', '$research', '$from', '$to', '$duration', '$budget', '$fund', '$Proposal_new_name', '$HGDG_new_name', '$paper_new_name', '$PIMME_new_name', 'unarchive', 'tbl_accomplished')";

        $insert_research_qry = mysqli_query($db, $insert_research);

        echo "<script>alert('New data has been added successfully');window.location.href = 'accomplished.php';</script>";
    }
}
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header modal-colored-header bg-primary">
                <h4 class="modal-title" id="primary-header-modalLabel">ADD GAD ACCOMPLISHED REPORT</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
            </div>

            <form action="accomplished.php" method="post" class="dropzone" id="myAwesomeDropzone" enctype="multipart/form-data">
